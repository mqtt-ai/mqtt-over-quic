...
The flow will survive disconnection or reconnection if the session and the flow are not expired (set by flow_expire_interval > 0).

The flow will also survives abortion if flow_expire_interval > 0.
...
Flow expiration means discard the flow state.

The flow state SHOULD NOT expire when it is not in shutdown states.

The flow state MUST expire after flow graceful shutdown.
...
Flow recover means that a previously aborted flow identified by Flow ID is restarted with their preserved state.
...
Flow recover success only when both endpoints holds the preserved state.
...
If the receiving side cannot successfully recover the flow state for any reason AND the `abort_if_no_state` bit is set,
it MUST abort the flow with the ERROR_NO_FLOW_STATE error code.

If the receiving side cannot successfully recover the flow state for any reason AND the `abort_if_no_state` bit is unset,
...
It is considered a protocol error (level 0) to attempt recovering a flow again if a previous attempt failed with the ERROR_NO_FLOW_STATE error code.
In such cases, the connection SHOULD be aborted with the ERROR_TOO_MANY_RECOVER_ATTEMPTS error code.
