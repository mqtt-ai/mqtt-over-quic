...
flow state information after the flow terminates unexpectedly.
...
Either side of the flow could abort the flow with ERROR_FLOW_OPEN_IDLE if the flow is idle after started this also including the scenario
that receiving timout of a complete stream header.
